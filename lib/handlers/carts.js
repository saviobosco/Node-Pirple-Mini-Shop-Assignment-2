...
handlers._carts.post = function(data, callback) {
    ...
    var menuId = typeof(data.payload.id) == "string" && data.payload.id > 0 ?
    data.payload.id : false;
    var quantity = typeof(data.payload.quantity) == "string" && data.payload.quantity > 0 ?
...
/**
 * Carts - GET : returns items in cart
 * required data : none
 * optional data : none
 * Authentication required
 */
...
/**
 * Carts - DELETE : remove item from cart
 * required data : menuId
 * optional data : none
 * Authentication required
 */
    ...
    if (menuItemId) {
        _data.read("carts", data.loggedInUser.email, function(err, userCart) {
            if (!err && userCart) {
                ...
                // get the removed item from the userCart 
